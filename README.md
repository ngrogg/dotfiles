...
* **sshConfigs**, SSH Config files.
  ...
  - **ssh-agent.service**, systemD Service for ssh-agent. Goes in `~/.config/systemd/user/ssh-agent.service`.
  - **ssh_askpass.conf**, SSH Ask Pass config. Goes in `~/.config/environment.d/ssh_askpass.conf`.
  - **ssh_auth_socket.conf**, SSH Auth Sock config.  Goes in `~/.config/environment.d/ssh_auth_socket.conf`.
